...
def roma_rakam_kontrol(kontrol_roma):
    ...
    for key in kontrol_roma:
        ...
        if roma_rakamlari.get(key) * 10 < roma_rakamlari.get(kontrol_roma[i]) and roma_rakamlari.get(
                key) < roma_rakamlari.get(kontrol_roma[i]):
...
def sadelestir(sayi1, sayi2):
                ...
                while sayi2.__len__() - 1 >= k and set(sayi2[k]).issubset(white_list_direk_sol):
                    ...
                    if unique.__len__() == 1 and unique2.__len__() == 0:
                        ...
                        for l in range(0, sayi1.__len__() - 1 + z):
                            if sayi2.__len__() > 0 and roma_rakamlari.get(sayi2[k]) * 10 >= roma_rakamlari.get(
                                    sayi1[sayi1.__len__() - l - 1]):
                                if roma_rakamlari.get(sayi2[k]) < roma_rakamlari.get(sayi1[sayi1.__len__() - l - 1]):
                                    sayi1 = replace_last(sayi1, sayi1[sayi1.__len__() - l - 1],
                                                         sayi2[k] + sayi1[sayi1.__len__() - l - 1])
                ...
                for p in range(0, len1):
                    if len2 > 0 and roma_rakamlari.get(sayi2[len2 - z - 1]) < roma_rakamlari.get(sayi1[len1 - p - 1]):
                        while roma_rakamlari.get(sayi2[len2 - z - 1]) * 10 <= roma_rakamlari.get(sayi1[len1 - p - 1]) \
                                or sayi2[len2 - z - 1] == "V" or sayi2[len2 - z - 1] == "L" or sayi2[
                                            len2 - z - 1] == "D":
                            if yeterli < 3 and buldum.__len__() == 0:
                                ...
                                if len1 - p - 2 > -1 and \
                                                roma_rakamlari.get(sayi1[len1 - p - 1]) == 5 * roma_rakamlari.get(
                                            sayi1[len1 - p - 2]) and \
                                                roma_rakamlari.get(sayi1[len1 - p - 1]) > roma_rakamlari.get(
                                            sayi1[len1 - p - 2]):
    ...
    if sayi2.__len__() > 0:
        ...
        sayi1, sayi2 = son_kontroller(sayi1, sayi2)
